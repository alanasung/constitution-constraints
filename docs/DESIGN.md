# Design Document: Constraint Checks for Multi-Rule Constitutions

## 1. Purpose

This document is the method contract for **Constraint Checks for Multi-Rule Constitutions**. It specifies what is
...
One-liner: Encode constitution-like rules as constraints and measure multi-dimensional compliance on local models.

Hypothesis: Encode constitution-like rules as constraints and measure multi-dimensional compliance on local models.
...
|---|---|---|
| Config schema | `constit.configs.schema` | Typed dataclasses + role-keyed models |
| Loader | `constit.configs.loader` | Hydra compose API |
| Device | `constit.models.device` | MPS/CUDA/CPU resolution + fallback flag |
| Registry | `constit.models.registry` | Pinned revisions + hardware fit |
| Hooks | `constit.models.hooks` | Capture, steer, ablate, patch, attn intervene |
| Generation | `constit.models.generation` | Chat template path recording |
| Data | `constit.data` | Synthetic items, splits, manifests |
| Evaluation | `constit.evaluation` | Metrics, failure harness, protocols |
| Ablation | `constit.ablation` | Control / dropout / seed-sweep arms |
| Reporting | `constit.reporting` | Aggregate → MD/TeX/figures |
| Cache | `constit.cache` | Versioned artifact store |
...
`src/constit/constit/` are preserved across regeneration.
